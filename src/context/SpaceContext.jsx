...
export const SpaceProvider = ({ children }) => {
    ...
    const fetchSpaces = useCallback(async () => {
        setLoading(true);
        try {
            const response = await fetch("https://space-backend-2-p4kd.onrender.com/spaces", {
                method: "GET",
    ...
    const updateSpace = async (spaceId, updatedData) => {
        ...
        try {
            const response = await fetch(`https://space-backend-2-p4kd.onrender.com/spaces/${spaceId}`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${authToken}`,
    ...
    const updateSpaceAvailability = async (spaceId, isAvailable) => {
        try {
            ...
            const response = await fetch(`https://space-backend-2-p4kd.onrender.com/spaces/${spaceId}/availability`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${authToken}`,
